...
- «тихие часы» (`MUTE_HOURS=23-07`): некритичные сообщения (успешная сделка,
  плановая статистика) ночью не будят владельца, а ошибки, низкий баланс и
  задержанные заказы проходят всегда;
- интерактивный бот владельца (long polling, без вебхука и открытых портов):
  `/help` `/status` `/stop` `/pause` `/resume` `/stats` `/report` `/tasks`
  `/balance` `/retry <id>` `/calc <₽> [звёзды]` `/limits` `/held` `/release <id>`
  `/blacklist [add|remove @nick]` `/customers [дней]`;
...
## 7b. Политика выдачи, шаблоны и ручные операции

Движок умел только «купить и выдать». Реальная торговля — это ещё и «не покупать»:
списанные USDT обратно не вернуть, поэтому все проверки считаются **до** запроса в
GAMEAU, а их итог — одно из трёх решений: `ignore` (живём как раньше), `alert`
(покупаем и предупреждаем), `hold` (не покупаем, заказ ждёт человека).

| Проверка | Параметр | Что делает |
|---|---|---|
| Крупная сделка | `MAX_ORDER_REVENUE_RUB` | выручка больше лимита → `HOLD_MANUAL` |
| Низкая маржа | `MIN_MARGIN_PCT` | маржа по курсу активного варианта ниже порога → `HOLD_MANUAL` |
| Суточный бюджет | `DAILY_SPEND_LIMIT_USDT` | «уже потрачено с полуночи + ещё эта покупка» больше лимита → `HOLD_MANUAL` |
| Дубль заказа | `DUPLICATE_WINDOW_MIN` + `DUPLICATE_ACTION` | тот же `@nick` и то же количество в окне → alert (по умолчанию) или hold |
| Стоп-лист | `BLACKLIST_ENABLED` + `--blacklist` | ник из `buyer_flags` → всегда hold |

Задержанный заказ — это **не** `FAILED`: оплата на FunPay уже есть, и `FAILED`
исказил бы статистику прибыльности. Статус `HOLD_MANUAL` не попадает в «зависшие»
(`STUCK_TASK_MINUTES` на него не кричит) и виден в `--stats` отдельной строкой
«⛔ задержано N».

Дальше решает человек:

```bash
autostars --held                          # кто, сколько и почему
autostars --release 1234567 -y             # выдать, вопреки политике (тот же пайплайн)
autostars --cancel 1234567 -y --note "ник не отвечает"   # отклонить (на FunPay — руками)
autostars --order @durov 1000 --dry-run    # ручная выдача вне сделки: смета + вердикт политики
autostars --limits [--json]                # что проверяется и сколько уже потрачено
```

Шаблоны ответов покупателю (`REPLY_NEED_USERNAME`, `REPLY_DELIVERED`, `REPLY_HOLD`,
`REPLY_ERROR`) живут в конфиге, а не в коде: значения по умолчанию совпадают с
текстами прежних версий, пустая строка = «не отправляем». Подстановки: `{username}`
(ник без `@`), `{quantity}`, `{quantity_spaces}`, `{order_id}`, `{price_rub}`,
`{reason}`; неизвестный ключ остаётся в тексте как есть и ничего не ломает.
Многострочные шаблоны в `.env` пишутся в двойных кавычках с `\n`.

Выгрузка и аналитика по покупателям:

```bash
autostars --export orders.csv --since 7d            # CSV в кодировке utf-8-sig (Excel)
autostars --export bad.csv --failed --with-events   # только проблемные + хронология событий
autostars --customers --days 30                     # оборот/провалы по каждому нику
```

Метрики (`METRICS_ENABLED=true`) — `GET /metrics` в формате Prometheus, `/healthz`
(503 при проблемах конфигурации) и `/status` (JSON). Реализация — свой лёгкий
HTTP-сервер на `asyncio` ([`autostars/metrics.py`](autostars/metrics.py)), новых
зависимостей нет; снимок пересчитывается раз в 15 с, scrape читает кэш.
По умолчанию выключено и слушает `127.0.0.1:9155`.

Модули: [`services/policy.py`](autostars/services/policy.py),
[`services/export.py`](autostars/services/export.py),
[`autostars/metrics.py`](autostars/metrics.py),
[`database/db_manager.py`](autostars/database/db_manager.py) (`buyer_flags`,
`sum_cost_usdt_since`, `find_recent_order_by_username`, `select_orders`) ·
тесты: `tests/test_risk.py`, `tests/test_templates.py`, `tests/test_export_metrics.py`.
...
Семь вкладок, все данные — из того же процесса/БД/логов, что у CLI:
...
| **Выдача** | расход USDT с полуночи против `DAILY_SPEND_LIMIT_USDT` и остаток, список задержанных заказов с кнопками «▶ отпустить» / «✖ отклонить» (обе — по второму клику), стоп-лист (добавить/убрать), выгрузка CSV за выбранный период и «только проблемные» |
| **Настройки** | 53 сгруппированных поля с подсказками, проверкой чисел/диапазонов, «💾 Сохранить в .env» (с бэкапом `.env.bak`, комментарии и порядок строк сохраняются), «↺ Перечитать»; шаблоны ответов — многострочные поля (форма сама экранирует переносы), а GUI-валидатор ругается на неизвестный `{плейсхолдер}` |
...
- вебхуков, админ-панели и «бота для чата с клиентами» — только исходящие соединения
  (единственное исключение — локальный `/metrics`, который ещё и выключен по умолчанию);
